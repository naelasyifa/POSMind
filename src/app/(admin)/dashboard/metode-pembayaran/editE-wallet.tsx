...
interface EditEWalletProps {
  ...
  onSave: (updatedEWallet: any) => void
}

export default function EditEWallet({ isOpen, onClose, ewalletData, onSave }: EditEWalletProps) {
  ...
  // Isi form dengan data ewallet yang dipilih
  useEffect(() => {
    if (ewalletData) {
      setNama(ewalletData.nama)
      setTipe(ewalletData.tipe)
      setStatus(ewalletData.status)
      setLimit(ewalletData.limit)
      setIntegrasi(ewalletData.integrasi)
    }
  }, [ewalletData])
  ...
  const handleSubmit = () => {
    if (nama && tipe) {
      const updatedEWallet = { nama, tipe, status, limit, integrasi }
      onSave(updatedEWallet)
      onClose()
    }
  }
    ...
    <>
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-gray-800">Edit Metode Pembayaran E Wallet</h2>
          <button
            onClick={onClose}
        ...
        <div className="space-y-4">
              ...
              <div className="relative">
                ...
                {isTipeDropdownOpen && (
                  <div className="absolute z-10 w-full mt-1 bg-white border border-gray-300 rounded-lg shadow-lg overflow-hidden">
                    <button
                      type="button"
                      onClick={() => handleTipeSelect('Static')}
                    ...
                    <button
                      type="button"
                      onClick={() => handleTipeSelect('Dynamic')}
                      className="w-full px-3 py-2.5 text-left text-gray-900 hover:bg-[#E0F7F6] transition-colors"
                    >
                      Dynamic
                    </button>
          ...
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1">
              Masukkan limit minimal (opsional)
            </label>
            <input
              type="text"
              value={limit}
