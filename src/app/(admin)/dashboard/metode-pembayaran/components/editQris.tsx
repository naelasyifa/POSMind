...
interface EditQrisProps {
  ...
  onSave: (updatedQris: any) => void
}

export default function EditQris({ isOpen, onClose, qrisData, onSave }: EditQrisProps) {
  ...
  // Isi form dengan data qris yang dipilih
  useEffect(() => {
    if (qrisData) {
      setNama(qrisData.nama)
      setTipe(qrisData.tipe)
      setStatus(qrisData.status)
      setLimit(qrisData.limit)
      setIntegrasi(qrisData.integrasi)
    }
  }, [qrisData])
  ...
  const handleSubmit = () => {
    if (nama && tipe) {
      const updatedQris = { nama, tipe, status, limit, integrasi }
      onSave(updatedQris)
      onClose()
    }
  }
    ...
    <>
      ...
      >
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-gray-800">Edit Metode Pembayaran Qris</h2>
          <button
            onClick={onClose}
